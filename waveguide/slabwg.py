...
class SlabWG:
    def __init__(self):
        ...
        self.epsilon[:, 14*N_FACTOR+1:18*N_FACTOR] = self.index * self.index * e0
        ...
        self.mHy0 = (self.mu - 0.5 * self.dt * self.Sigma_mux) / (self.mu + 0.5 * self.dt * self.Sigma_mux)
        self.mHy1 = (self.dt/self.da) / (self.mu + 0.5 * self.dt * self.Sigma_mux)
        self.mHx0 = (self.mu - 0.5 * self.dt * self.Sigma_muy) / (self.mu + 0.5 * self.dt * self.Sigma_muy)
        self.mHx1 = (self.dt/self.da) / (self.mu + 0.5 * self.dt * self.Sigma_muy)

        # E field update coeficient
        self.mEzx0 = (self.epsilon - 0.5 * self.dt * self.Sigma_x) / (self.epsilon + 0.5 * self.dt * self.Sigma_x)
        self.mEzx1 = (self.dt/self.da) / (self.epsilon + 0.5 * self.dt * self.Sigma_x)
        self.mEzy0 = (self.epsilon - 0.5 * self.dt * self.Sigma_y) / (self.epsilon + 0.5 * self.dt * self.Sigma_y)
        self.mEzy1 = (self.dt/self.da) / (self.epsilon + 0.5 * self.dt * self.Sigma_y)
        
        # The fig, ax and Ez_prop here is to visualization.
    ...
    def update_E(self, t):
        ...
        # add source
        self.Ezx[bound_width, 14*N_FACTOR+1:18*N_FACTOR] = 0.5*np.sin(2*np.pi*(c0/(self.da*self.N_lamdba))*t*self.dt)
        self.Ezy[bound_width, 14*N_FACTOR+1:18*N_FACTOR] = 0.5*np.sin(2*np.pi*(c0/(self.da*self.N_lamdba))*t*self.dt)
    ...
    def init_pml(self):
        ...
        # 先算出Sigama_max 
        sigmamax = (6 * (grid_order + 1) * e0 * c0) / (2 * bound_width * self.da)
        # 因为我们是2D-FDTD，所以一共有2个Boundary factor: bf_x, bf_y
        bf_x = ((self.epsilon[int(self.X_dim/2), bound_width] / e0) * sigmamax) / ((bound_width ** grid_order) * (grid_order + 1))
        bf_y = ((self.epsilon[bound_width, int(self.Y_dim/2)] / e0) * sigmamax) / ((bound_width ** grid_order) * (grid_order + 1))
        
        # 计算 PML层的参数，我这里两边都取一样的宽度
        slice_x = bf_x * \
        (
            (np.arange(bound_width+1)+0.5*np.ones(bound_width+1))**(grid_order+1) - 
            (np.arange(bound_width+1)-0.5*np.concatenate((np.array([0]), np.ones(bound_width))))**(grid_order+1) 
        )
...
if __name__ == "__main__":
    ...
    def update(n):
        image = plt.imshow(Ez_prop[n], interpolation='bilinear', vmax=1, vmin=-1, cmap='seismic')
        plt.title("FDTD with PML waveguide simulation, {:.1f}fs".format(dt * n * 20 * 1e15))
        plt.xlabel("x (0.05 um)")
        plt.ylabel("y (0.05 um)")
        # 将这一帧的照片也保存下来
        plt.savefig(os.path.join('results', 'slab_waveguide', str(n*20)+'.png'))
        return image,
    
    fig = plt.figure()
    animation = ani.FuncAnimation(fig, update, frames=range(len(Ez_prop)), blit=True)
    animation.save(os.path.join(path, 'Slab_WG.gif'), writer=ani.PillowWriter(fps=5))
