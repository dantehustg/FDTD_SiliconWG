...
class FDTD1dGrid:
    ...
    def __init__(self, source: GaussSource, device: SlabDevice, visualize=False):
        ...
        self.ER = np.ones(self.Nz)
        self.UR = np.ones(self.Nz)
        self.applied_device()

        # FDTD fields and update coefficients
        self.mEy = (c0 * self.dt) / self.ER
        self.mHx = (c0 * self.dt) / self.UR
        self.Ey = np.zeros(self.Nz)
        self.Hx = np.zeros(self.Nz)
        ...
        self.normH = - np.sqrt(self.ER[self.nz_src] / self.UR[self.nz_src])

        # used for show the results
        self.fig, self.ax = plt.subplots()
        self.za = np.arange(self.Nz) * self.dz
